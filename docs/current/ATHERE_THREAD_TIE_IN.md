...
HEAD: check git log -1 (Item 24 distributed state may be local until pushed).
...
- Item 24 (distributed state layer) — see ACTIVE_RUN.
- Deferred: mission-hash skill library (checkpoint 81) — do not start unless re-ordered.
- 24-item backlog Phase 11 complete when Item 24 is acceptance-audited in ACTIVE_RUN.

First move every turn with material work:
List skills/tools you are loading → Read them → Then act.
Wait for Justin’s next order. Do not start coding until ordered.
```
...
- Live checkpoint remains `runtime/ATHERE_ACTIVE_RUN.md`.
