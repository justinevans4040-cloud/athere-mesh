...
test('superseded authoritative facts are hidden from normal agent state and remain available only through explicit history', async () => {
  const root = await mkdtemp(path.join(tmpdir(), 'athere-state-supersession-'));
  const service = createMissionStateService({ root, clock });
  const input = createInput();
  input.authoritativeFacts = [{ id: 'server-ip-v3', key: 'SERVER_IP', value: '100.64.0.10', status: 'current' }];
  const created = await service.create(input);

  const updated = await service.transition({
    missionId: created.mission.id,
    expectedRevision: created.revision,
    signal: { type: 'running', agent: 'nyx', detail: 'verified server address changed' },
    update: {
      authoritativeFacts: [
        {
          id: 'server-ip-v3',
  ...
  assert.deepEqual((await service.facts({ missionId: created.mission.id, key: 'SERVER_IP', includeHistorical: true })).map(({ id, status }) => ({ id, status })), [
    { id: 'server-ip-v3', status: 'superseded' },
    { id: 'server-ip-v4', status: 'current' },
  ]);

  const selected = await service.select({ missionId: created.mission.id, fields: ['currentFacts'] });
  assert.deepEqual(selected.currentFacts.map(({ id }) => id), ['server-ip-v4']);
  await assert.rejects(
    service.select({ missionId: created.mission.id, fields: ['authoritativeFacts'] }),
    /unsupported selected state field: authoritativeFacts/,
  );

  const history = await service.history({ missionId: created.mission.id });
  assert.deepEqual(history[1].changes.authoritativeFacts.before, [
    { id: 'server-ip-v3', key: 'SERVER_IP', value: '100.64.0.10', status: 'current' },
  ]);
  assert.equal(history[1].changes.authoritativeFacts.after[0].status, 'superseded');
  assert.equal(history[1].changes.authoritativeFacts.after[1].supersedes, 'server-ip-v3');
  assert.equal((await service.verifyHistory({ missionId: created.mission.id })).valid, true);
});

test('fact lineage validation rejects ambiguous current state and broken supersession links', async () => {
  const root = await mkdtemp(path.join(tmpdir(), 'athere-state-fact-guard-'));
  const service = createMissionStateService({ root, clock });
  const created = await service.create(createInput());
  ...
  await assert.rejects(
    ...
    /multiple current authoritative facts for key: SERVER_IP/,
  );
  ...
  await assert.rejects(
    service.transition({
      ...
      update: {
        authoritativeFacts: [
          { id: 'revoked-ip', key: 'SERVER_IP', value: '10.0.0.1', status: 'revoked' },
        ],
      },
    }),
    /revoked fact revoked-ip requires revokedAt/,
  );
});
