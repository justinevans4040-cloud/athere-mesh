...
- Current focus: **Ckpt 91** — doctrine A→B: shared-store boot recovery + owner smoke re-proven on current orchestrator (executor `state_version >=` fix).
...
87. **Dual hostile audit OPEN closes (local).** Source-doc + break audits found 3 proven OPEN holes; closed test-first.

- **Holes found OPEN and closed:**
  1. Item 24 `Symbol.for` brand forgeable → **WeakSet** instance registry (not global symbol).
  2. Revoked identity still submitted learning / proposed improvements → `identities.assertActive` on submit/propose/approve/deploy/monitor.
  3. Method-shape learning/improvement injection bypassed SoD → WeakSet factory brands required by `createMissionStateService`.
- **Tests:** `tests/integration/athere-hostile-probe-item24-brand-forge.test.js` now GREEN (was RED evidence).
- **Suite after close:** 427 pass / 0 fail / 17 skip.
- Residual: custom `store` still trusted composition; source-doc acceptance gaps (Item 1 baseline, Item 2 frozen control file, Item 21 measured performance, Item 16 orchestrator wire, Item 24 physical distribution) remain separate from these security closes.

88. **Ruthless hostile audit OPEN closes (local).** Round-2 break audit found 6 new OPEN holes after ckpt 87; closed test-first.

- **Holes found OPEN and closed:**
  1. Unbranded identities registry injection → WeakSet brand required.
  2. Branded learning with hostile identities stub → factory requires branded identities + service requires same registry instance.
  3. Branded improvement with hostile identities stub → same bind.
  4. Unbranded skills library injection → WeakSet brand required.
  5. Skill library accepted forged `listPermanent` → requires branded learning pipeline.
  6. Unregistered `ghost_writer` recordFact via has-skip → always `identities.assertActive` (unknown REJECT).
- **Tests:** `tests/integration/athere-hostile-probe-ruthless-ckpt87.test.js` GREEN (14/14).
- **Suite after close:** 441 pass / 0 fail / 17 skip.
- Residual: custom `store` still trusted composition; stolen real branded objects remain usable by design; source-doc HIGH gaps unchanged.

89. **Ruthless hostile audit OPEN closes (local, round 3).** Ckpt 87/88 named closes held; two new OPEN holes closed test-first.

- **Holes found OPEN and closed:**
  1. Branded skills from registry/learning A injected into service with learning B → require `getValidatedSkillLearning(skills) === service.learning`.
  2. Learning `store()` after approver revoke still wrote permanent knowledge → `identities.assertActive(approvedBy)` on `store()`.
- **Tests:** `tests/integration/athere-hostile-probe-ruthless-ckpt88.test.js` GREEN (was 2 RED).
- **Suite after close:** 462 pass / 0 fail / 17 skip.
- Residual: custom `store` trusted composition; stolen branded objects; source-doc HIGH gaps (Items 1, 2, 16, 21, 24) unchanged.

90. **Source HIGH gaps closed (Items 1, 2, 16, 21, 24).** Ordered close after Justin go; test-first where behavioral.

- **Item 1:** Refreshed `docs/current/ATHERE_ARCHITECTURE_BASELINE_2026-08-26.md` to match HEAD (Items 2–24 packages, MEA, harness SHA, mesh fabric, honest residuals).
- **Item 2:** Restored LF-canonical frozen control bytes; pinned SHA `08f56024…` via `loadFrozenEvaluationControl` + `.gitattributes eol=lf`; contract `evaluation-frozen-control.test.js`.
- **Item 21:** Learning compare/measure reject caller-supplied metrics; require Item 2 harness `improvement_proven` vs frozen control.
- **Item 16:** Orchestrator `blockThenHeal` consults `decideNext({ actor: 'orchestrator' })` and applies recovery strategy or escalates; `executive-orchestrator-item16.test.js`.
- **Item 24:** Optional `durableReplicaDir` for cross-process replica capacity reads; primary remains sole writer; `distributed-state-item24-durable.test.js`.
- **Suite:** 467 pass / 0 fail / 17 skip.
- Residual: custom `store` trusted composition; Item 24 still not geo/CRDT/failover; durable replicas are capacity reads only.

91. **Doctrine A→B path close (ordered after hostile audit).** Boot recovery + current-tree owner smoke.

- **Boot recovery:** `createTitanService` resolves mesh deps **before** `recoverAndHealMissions`; passes `missionStore: mesh.store` when shared Postgres is wired. Stores expose `listMissionIds` (filesystem + Postgres). Hermetic: `tests/integration/boot-recovery-shared-store.test.js`.
- **Regression found live:** post-inspect checkpoints bumped `state_version` above hard-coded rune `=== 3` → worker rejected `execute_node_tests`. Closed: `state_version >= missionStateVersion` in `node-test-executor.js`; synced to Ichabod worker + unit restart (MainPID `2576714`).
- **Current-tree smoke:** `evidence/smoke-owner-api-mission-crosshost-20260904T214254.json` — `ok: true`, wiring redis+queue+postgres, mission `mission-7c820439-59e8-45b5-bc3f-d851911ae666` revision **7** `completed` verified in Ichabod `titan_missions`, remote suite 310/309/0/1, `midFlightSshClaim: false`. Orchestrator SHA `a85835e37049…` matches evidence.
- **Still residual:** HTTP `/api/commands` cross-host not smoked; proofs remain owner-local FS; Postgres still tunnel-bound; worker checkout may lag Lenovo HEAD.
